...
A function usually gets one or more inputs called *arguments*. Functions often (but not
always) return a *value*. 


``` r
# The following are not real functions – they won't run!

function(argument1, argument2, argument3, ...) # Returns a value in the console
value <- function(argument1, argument2, argument3, ...) # Saves the value in the environment
value <- function(named_argument = input1) # Provides an input for a specific argument to the function
```
